...
__all__ = ('default_log_colors', 'ColoredFormatter', 'LevelFormatter')
...
default_formats = {
    '%': '%(timestr)s %(log_color)s%(levelname)s %(namestr)s:%(fileinfo)s %(msgstr)8s jj',
    '{': '{log_color}{levelname}:{namestr}:{message} kk',
    '$': '${log_color}${levelname}:${namestr}:${message} ll'
}

DEFAULT_LEVELS = {
    'DEBUG': '%(timestr)s %(log_color)s%(levelname)s %(namestr)s:%(fileinfo)s %(msgstr)8s',
    'INFO': '%(timestr)s %(log_color)s%(levelname)s %(fileinfo_squashed)s %(msgstr)8s',
    'WARNING': '%(timestr)s %(log_color)s%(levelname)s %(fileinfo_squashed)s %(msgstr)8s',
    'ERROR': '%(timestr)s %(log_color)s%(levelname)s %(fileinfo_squashed)s %(msgstr)8s',
    'CRITICAL': '%(timestr)s %(log_color)s%(levelname)s %(fileinfo_squashed)s %(msgstr)8s',
}
...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        record.namestr = record.name.ljust(10)
        record.fileinfo = "%s:%s:%s" % (record.filename, str(record.lineno).ljust(4), record.funcName.ljust(30))
        record.fileinfo_squashed = "%s:%s:%s" % (record.filename, str(record.lineno), record.funcName.ljust(10))
        split_str = os.path.dirname(record.pathname).split('/')
        record.module_dir = split_str[len(split_str)-1]
        record.msgstr = (record.msg % record.args).rstrip()
...
class LogstashFormatter(logging.Formatter):
    def format(self, record):
        ...
        """
        msg = super(LogstashFormatter, self).format(record)

        now_str = datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ')
        host = get_host()

        # TODO: exc_info, if we use it
